...
def train_evaluate_register(preprocessing_run_id, C=1.0):
    """
    Loads preprocessed Breast Cancer data, trains a model, evaluates it, and
    ...
    with mlflow.start_run(run_name=f"logistic_regression_C_{C}"):
        ...
        mlflow.sklearn.log_model(pipeline, "breast_cancer_classifier_pipeline")

        # 5. ตรวจสอบและลงทะเบียนโมเดล
        if acc >= ACCURACY_THRESHOLD:
            print(f"Model accuracy ({acc:.4f}) meets the threshold. Registering model...")
            model_uri = f"runs:/{mlflow.active_run().info.run_id}/breast_cancer_classifier_pipeline"
